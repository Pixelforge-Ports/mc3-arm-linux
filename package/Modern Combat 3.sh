...
export MC3_RESOLUTION="${MC3_RESOLUTION:-auto}"
if [ -f "$GAMEDIR/resolution.txt" ]; then
  export MC3_RESOLUTION="$(tr -d '\r\n' < "$GAMEDIR/resolution.txt")"
fi
...
EOF
  ...
  if command -v pm_message >/dev/null 2>&1; then
    pm_message "Modern Combat 3: preparing game data. Keep the device powered on."
...
gl_mesa_vendor_loads() {
  ...
  LD_LIBRARY_PATH="$GL_SHIM:$LD_LIBRARY_PATH" \
    "$GAMEDIR/mc3" --gl-probe "$GL_MESA_DIR/libEGL_mesa.so.0" >/dev/null 2>&1
}
...
gl_mesa_unshadow() {
  ...
  "$GAMEDIR/mc3" --gl-probe-deps "$GL_MESA_DIR/libEGL_mesa.so.0" 2>&1 | sed 's/^/GL:   /'
...
mapper_pid=
if [ -n "${GPTOKEYB2:-}" ]; then
  $GPTOKEYB2 "mc3" -c "$GAMEDIR/mc3.ini" &
  mapper_pid=$!
fi
