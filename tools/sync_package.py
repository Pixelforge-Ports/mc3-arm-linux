...
def sync():
    ...
    for name in ["port.json", "README.md", "gameinfo.xml", "cover.png", "screenshot.png"]:
        shutil.copyfile(ROOT / "ports/mc3" / name, package / name)
    shutil.copyfile(ROOT / "ports/Modern Combat 3.sh", package / "Modern Combat 3.sh")
    for name in ["mc3.ini", "mc3.eapx.json", "CREDITS.md", "PUT_MC3_DATA_HERE.txt"]:
        shutil.copyfile(ROOT / "ports/mc3" / name, data / name)
    ...
    for origin, name in [("LICENSE", "LICENSE-portmaster-port.txt"), ("LICENSE", "LICENSE-eapx.txt"),
                         ("NOTICE.md", "NOTICE.md"), ("ports/mc3/LICENSE-gptokeyb.txt", "LICENSE-gptokeyb.txt")]:
    ...
    shutil.copyfile(ROOT / 'portbase/third_party/gmloader/LICENSE.md', licenses / 'LICENSE-gmloader.md')
    shutil.copyfile(ROOT / 'portbase/third_party/powervr/LICENSE.md', licenses / 'LICENSE-powervr.txt')
    shutil.copyfile(ROOT / 'portbase/third_party/stb/LICENSE.md', licenses / 'LICENSE-stb.md')
