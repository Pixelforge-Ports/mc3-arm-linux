...
try {
    $buildArguments = @('build', '--progress=plain', '-t', 'mc3-build', '-f', 'portbase/Dockerfile.build')
    if ($NoCache) { $buildArguments += '--no-cache' }
    $buildArguments += 'portbase'
    & $dockerCommand.Source @buildArguments
    if ($LASTEXITCODE -ne 0) { throw 'Docker build environment failed; see the error above.' }
    & $dockerCommand.Source run --rm --mount "type=bind,source=$PSScriptRoot,target=/src" -w /src mc3-build bash -lc 'make -j2 && make libs && bash package_portmaster.sh'
    if ($LASTEXITCODE -ne 0) { throw 'Port compilation or packaging failed; see the error above.' }
    Write-Host "Built: $PSScriptRoot/build/mc3-native-arm.zip"
