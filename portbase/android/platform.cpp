...
/*
 * Two ways to speak to this game, selected by <PREFIX>_INPUT. Joystick is the
...
static bool input_is_joystick(void)
{
    static int mode = -1;
    if (mode < 0) {
        const char *env = port_getenv("INPUT");
 ...
 * <PREFIX>_FACE_LAYOUT=xbox switches them for a handheld lettered the other way.
 */
...
static int32_t map_button(int button)
{
     ...
     * build like this OUYA one expects. <PREFIX>_FACE_KEYS=legacy restores the
 ...
 * because that is what these devices ship with; <PREFIX>_FACE_LAYOUT=xbox
...
static bool face_keys_are_legacy(void)
{
    static int legacy = -1;
    if (legacy < 0) {
        const char *env = port_getenv("FACE_KEYS");
...
{
    /* <PREFIX>_CURSOR_TEST parks the cursor on screen with no pad attached,
    ...
    static int forced = -1;
    if (forced < 0)
        forced = port_getenv("CURSOR_TEST") ? 1 : 0;
 ...
 * hardware attached. <PREFIX>_SYNTH_INPUT=<ms> taps the screen centre at that
...
static void synth_input_tick(void)
{
    ...
    if (interval < 0) {
        const char *env = port_getenv("SYNTH_INPUT");
...
extern "C" int ALooper_pollAll(int timeoutMillis, int *outFd, int *outEvents, void **outData)
{
    ...
    {
        static long calls = 0;
        static bool probe = port_getenv("POLL_PROBE") != NULL;
...
{
    /* <PREFIX>_INPUTTRACE=1 records which axes the game actually asks for. An
