...
/* Defined by the port, in its own main.cpp, with C++ linkage - so declared
 * outside the extern "C" block below or the two would not be the same symbol.
 * This is part of the contract in docs/PORT-CONTRACT.md: portbase needs a
 * handle on the loaded guest module to walk its exports. */
struct so_module;
extern so_module *port_guest_module(void);
 ...
 * sometimes. <PREFIX>_LATE_THREAD_MS exists so the next person can find out
...
{
    so_module *mod = port_guest_module();
    if (!mod || !entry)
        return false;
    ...
    const char *want = port_getenv("LATE_THREAD_AT");
    if (!want || !*want)
...
{
    const char *env = port_getenv("LATE_THREAD_MS");
    /*
     ...
     * single thread that matters is what <PREFIX>_LATE_THREAD_AT is for, and 0
    ...
    trace("  ^ delayed %d ms before starting (engine startup race; "
          "" PORT_ENV_PREFIX "_LATE_THREAD_MS=0 disables)", ms);
 ...
 *
 * (the guest library ships it at .data+0x1210c0, word = 0x4000), and the engine's
 ...
 * still showing nothing but "Linking the guest library...".
 *
...
int bionic_pthread_mutex_init(BIONIC_pthread_mutex_t *m, pthread_mutexattr_t **attr)
{
     ...
     * word the game reserved. the guest library imports no pthread_mutexattr_*
