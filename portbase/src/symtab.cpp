/*
 * The import tables the bionic ELF loader binds into the guest library.
 *
...
extern DynLibFunction symtable_cxx[];       /* src/symtab_cxx.cpp           */
 ...
 *
 * the guest library declares nine - libfmodex, libz, liblog, libEGL,
 ...
 *
 * libstdc++.so is answered by src/symtab_cxx.cpp. Whether it is consulted
 * depends on the game: one that links its C++ runtime statically imports no
 * mangled name at all, and several of these engines do exactly that. One that
 * does not imports the four allocation operators and nothing else - that is
 * the whole public surface of the NDK's libstdc++ stub.
 *
...
DynLibFunction *so_dynamic_libraries[] = {
    ...
    symtable_libm,
    symtable_zlib,
    /* bionic's libstdc++.so: the four allocation operators. Games that link
     * their C++ runtime statically - which was every port before this one -
     * import none of them, so the table is simply never consulted. */
    symtable_cxx,
    symtable_libc,
    NULL,
};
