 ...
 */
extern "C" int portbase_open(const char *path, int flags, ...)
{
    mode_t mode = 0;
...
extern "C" int portbase_ioctl(int fd, int request, ...)
{
    va_list ap;
    va_start(ap, request);
 ...
 */
extern "C" void portbase_assert2(const char *file, int line,
...
DynLibFunction symtable_libm[] = {
    ...
    /* The first game to import a hyperbolic. Its siblings (coshf, tanhf and
     * the double forms) are deliberately not added alongside it: an import
     * nothing declares is dead weight, and an import that IS declared is
     * reported by name before a single instruction runs, which is a cheaper
     * way to learn what is needed than guessing. */
    THUNK_SPECIFIC("sinhf",  HOST_MATH::sinhf),
    THUNK_SPECIFIC("sqrtf",  HOST_MATH::sqrtf),
    THUNK_SPECIFIC("tanf",   HOST_MATH::tanf),

    NO_THUNK("open",  (uintptr_t)&portbase_open),
    NO_THUNK("ioctl", (uintptr_t)&portbase_ioctl),
    /* No float crosses the boundary, so no ABI bridge is needed. */
    NO_THUNK("__assert2", (uintptr_t)&portbase_assert2),
