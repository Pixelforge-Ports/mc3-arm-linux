      ...
      for (int j = 0; j < mod->num_dynamic; j++) {
        switch (mod->dynamic[j].d_tag) {
          case DT_NEEDED:
             ...
             *
             *   D:/<studio-build-path>/Packages/.../android-ndk-r7/
 ...
 *
 *     arm-linux-gnueabihf-objdump -d --start-address=0x<off> the guest library
 *
...
void so_initialize(so_module *mod) {
  ...
  for (int i = 0; i < mod->num_init_array; i++) {
    ...
    // one. the guest library has exactly such a slot: the last entry of its
