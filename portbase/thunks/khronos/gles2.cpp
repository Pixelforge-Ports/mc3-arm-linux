...
static void port_discard(GLenum, GLsizei, const GLenum *) {}
static void port_bind_vao(GLuint) {}
static void port_gen_vao(GLsizei n, GLuint *a) { for (GLsizei i=0; i<n && a; ++i) a[i]=i+1; }
static void port_delete_vao(GLsizei, const GLuint *) {}
static GLboolean port_is_vao(GLuint) { return GL_FALSE; }
static void *port_map_buffer(GLenum, GLenum) { return nullptr; }
static GLboolean port_unmap_buffer(GLenum) { return GL_FALSE; }
 ...
 * <PREFIX>_SHADER_DUMP_DIR, 16 programs, ids 6..51):
 *
 *     #ifndef COLOR_CURVES
 *     #define COLOR_CURVES
 *     #endif
 ...
 * <PREFIX>_DISABLE_FRAMEBUFFER_FETCH, which only the emulator sets.
 */
static void port_neutralise_framebuffer_fetch(std::string &source)
{
    ...
    const struct { const char *from; const char *to; } patches[] = {
        ...
        { "#define FRAMEBUFFER_RGBA gl_LastFragData[0]",
          "lowp vec4 port_fbfetch() { discard; return vec4(0.0); }\n"
          "#define FRAMEBUFFER_RGBA port_fbfetch()" },
    };
...
void glShaderSource_dump(
	...
	const bool disable_fb_fetch = port_getenv("DISABLE_FRAMEBUFFER_FETCH") != nullptr;
    ...
    if (disable_fb_fetch)
        port_neutralise_framebuffer_fetch(source);
...
void load_gles2_funcs()
{
	...
	glad_glDiscardFramebufferEXT = (PFNGLDISCARDFRAMEBUFFEREXTPROC)port_discard;
	if (!glad_glBindVertexArrayOES) glad_glBindVertexArrayOES = (PFNGLBINDVERTEXARRAYOESPROC)port_bind_vao;
	if (!glad_glGenVertexArraysOES) glad_glGenVertexArraysOES = (PFNGLGENVERTEXARRAYSOESPROC)port_gen_vao;
	if (!glad_glDeleteVertexArraysOES) glad_glDeleteVertexArraysOES = (PFNGLDELETEVERTEXARRAYSOESPROC)port_delete_vao;
	if (!glad_glIsVertexArrayOES) glad_glIsVertexArrayOES = (PFNGLISVERTEXARRAYOESPROC)port_is_vao;
	if (!glad_glMapBufferOES) glad_glMapBufferOES = (PFNGLMAPBUFFEROESPROC)port_map_buffer;
	if (!glad_glUnmapBufferOES) glad_glUnmapBufferOES = (PFNGLUNMAPBUFFEROESPROC)port_unmap_buffer;
}
