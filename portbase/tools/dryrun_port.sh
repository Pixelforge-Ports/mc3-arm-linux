...
#   * "/$directory/ports/${PORT_NAME}" resolves to where the port was installed
#   * cd, the log.txt redirect and mkdir of ${ENV_PREFIX}_DATA_DIR all succeed
...
#       ${BUILD_IMAGE} tools/dryrun_port.sh /game
set -uo pipefail

# Port identity (PORT_NAME, ENV_PREFIX, BUILD_IMAGE) from emulator.json.
. "$(dirname "${BASH_SOURCE[0]}")/../tools/port_config.sh"

DONOR="${1:?usage: dryrun_port.sh <path to Real Racing 3 APK, ZIP or extracted tree>}"
ROOT=/tmp/dryrun
PM="$ROOT/home/.local/share/PortMaster"
GAMEDIR="/roms/ports/${PORT_NAME}"
...
cp "build/${PORT_NAME}"                  "$GAMEDIR/${PORT_NAME}"
cp "ports/${PORT_NAME}/${PORT_NAME}.gptk"   "$GAMEDIR/${PORT_NAME}.gptk"
cp "ports/${PORT_NAME}/${PORT_NAME}.eapx.json" "$GAMEDIR/${PORT_NAME}.eapx.json"
cp "ports/${PORT_NAME}/cover.png"        "$GAMEDIR/cover.png"
...
MISSING=""
covq=$(needed_of "$GAMEDIR/${PORT_NAME}"); covseen=" "
while [ -n "${covq// /}" ]; do
    covnext=""
    for s in $covq; do
...
chmod -x "$GAMEDIR/${PORT_NAME}"   # prove the launcher's chmod +x is doing work
...
export ${ENV_PREFIX}_FRAME_LIMIT="${${ENV_PREFIX}_FRAME_LIMIT:-40}"
...
check "eapx marker written"                       test -f "$GAMEDIR/.eapx-${PORT_NAME}-data.json"
check "eapx import log written"                   test -s "$GAMEDIR/eapx.log"
check "donor native library imported"             test -f "$GAMEDIR/${GUEST_SO}"
...
check "binary made executable"                   test -x "$GAMEDIR/${PORT_NAME}"
